...
|| 16 | **Probe-vs-probe identical score anomaly** | ✅ RESOLVED (superseded) | Original fix hardcoded a fallback score of 85.0 for identical templates. Removed 2026-08-30 — proper root-pair enumeration + BFS pairing growth now produces a natural score of 88.3 for probe-vs-probe without any special case. |\
|| 17 | **Root pair enumeration without dedup or limits** | ✅ RESOLVED | matcher/mod.rs — HashSet dedup on (probe_ref, cand_ref) root pairs. MinRootEdgeLength=58. MAX_TRIED_ROOTS=70, MAX_ROOT_LOOKUPS=1633. Additionally fixed 2026-08-30: root enumeration now seeds a SINGLE MinutiaPair per root (mirroring .NET RootEnumerator/EdgeSpider.Crawl) instead of two, and EdgeHashes lookup direction corrected to build the hash table on PROBE and iterate CANDIDATE edges (mirrors .NET `Hash = EdgeHashes.Build(probe)` in FingerprintMatcher.cs). |
|| 18 | **grow_pairing infinite loop / hang on self-similar templates** | ✅ RESOLVED | matcher/mod.rs `grow_pairing()` — rewrote from a batch-collect-then-append loop (which could re-add conflicting pairs every pass and hang on probe-vs-probe) to a proper BFS queue with used-probe/used-candidate index sets, mirroring .NET `EdgeSpider.Crawl()`+`CollectEdges()`. |
|| 19 | **Cross-image matching collapsed to near-zero score** | ⚠️ ACTION NEEDED | Root cause narrowed down 2026-08-30: (a) `build_sorted_neighbor_edges` sorted edges DESCENDING by length — fixed to ASCENDING to match .NET `NeighborEdge.BuildTable`. (b) `MinutiaPair` was missing `ProbeRef`/`CandidateRef` fields — .NET's `Scoring.Compute()` rebuilds each pair's edge from ITS OWN discovery reference (the pair that found it during BFS crawl), not the global root; added `probe_ref`/`candidate_ref` to `MinutiaPair`, set them in `grow_pairing()`, and rewired `compute_score()` to use them. Real-image debug (see `MATCHER_DEBUG` trace, now removed) shows the pairing growth for probe.png vs matching.png (49 vs 55 minutiae) only reaches 3-6 paired minutiae before stalling — nowhere near the 20-40+ needed for score ≥40. Remaining root cause: `supporting_edges` on `MinutiaPair` is NEVER incremented anywhere in the Rust port — .NET's `PairingGraph.Support()` (called from `EdgeSpider.CollectEdges` whenever a rediscovered pair's minutiae are already claimed) is entirely unimplemented. This "support" mechanism is what lets .NET's BFS keep expanding into an already-dense pairing region instead of stalling once neighbor sets get exhausted — needs a full `PairingGraph`/`ByProbe`/`ByCandidate`/`Support()` port (see `/tmp/sourceafis-net/SourceAFIS/Engine/Matcher/PairingGraph.cs` and `EdgeSpider.cs` lines 55-59) before cross-image score parity is achievable. |
...
|| 6 | **shape_score interpolation table wrong** | ✅ RESOLVED (2026-08-30) | `src/matcher/mod.rs::shape_score` had wrong (start,length) pairs vs .NET `Scoring.Shape()` — corrected to match `[8.48,11.12)->(0,3)`, `[11.12,14.15)->(3,7)`, `[14.15,18.22)->(10,10)`, `[18.22,22.39)->(20,10)`, `[22.39,27.24)->(30,10)`, `[27.24,32.01)->(40,10)`, `>=32.01`->linear above 50. probe-vs-probe raw score improved from 0/hang to shaped 88.34 (still below .NET's uncapped ~548 — see #7). |
|| 7 | **Score scale mismatch: .NET has no 0-100 cap** | ⚠️ ACTION NEEDED | Ran real .NET SourceAFIS (dotnet-sdk-8.0 installed, `/tmp/sourceafis-net`) via a diagnostic CLI (`/tmp/sourceafis-score-cli`). .NET scores are **uncapped**: probe-vs-probe=548.40, probe-vs-matching=235.56, probe-vs-nonmatching=3.23. Rust's `shape_score` clamps/interpolates into a 0-100-ish range by design, so absolute magnitudes will never match .NET 1:1 — only relative ordering and threshold behavior (≥40 match / ≤20 non-match, scaled to Rust's own range) are meaningful comparisons. Document this explicitly before adding more "must equal .NET score" assertions. |
|| 8 | **Minutiae count divergence from .NET (root cause of cross-image collapse)** | ⚠️ ACTION NEEDED | Real .NET reference counts (via reflection on `FingerprintTemplate.Minutiae`): probe.png=46, matching.png=37, nonmatching.png=28, probe.jpeg=45. Rust counts on the same files: probe.png=49 (close), matching.png=55 (+18 vs .NET), nonmatching.png=**89** (+61, more than 3x .NET), probe.jpeg=**100** (hit `TopMinutiaeFilter`'s hard cap — true count is likely much higher than .NET's 45). Root cause: `MinutiaCollector` in Rust (`src/extractor/minutia_collector.rs`) counts raw pixel neighbors in the binarized/skeleton bitmap (`neighbor_count<=1`=ending, `>=3`=bifurcation) instead of walking the .NET-style skeleton graph (`Skeleton.Minutiae` nodes where `minutia.Ridges.Count==1` or `>=3`, built from `SkeletonGraphs`/ridge tracing). These are structurally different algorithms, not a tunable-constant bug — the Rust approach over-detects spurious minutiae on noisy/high-contrast regions (worst on nonmatching.png, which apparently binarizes noisier). This inflated/wrong minutiae set is the most likely reason `grow_pairing`'s BFS can't find enough correct correspondences on real images (issue previously attributed solely to missing `PairingGraph.Support()` — that is still true and blocking, but minutiae quality is a compounding, possibly primary, cause). |
|| 9 | **Extraction pipeline ~5-6x slower than .NET** | ⚠️ ACTION NEEDED (partially mitigated) | Measured via `EXTRACT_PROFILE=1` env var (`src/extractor/mod.rs::extract`, per-stage `println!` timing) on probe.png (388x374): `local_histograms` stage was **~400-475ms**, ~95% of total per-image extraction time. Root cause: `LocalHistograms::new` (`src/extractor/local_histograms.rs`) computes a *per-pixel* 256-bin histogram over a 15x15 window naively — O(w·h·window²). Reference `.NET LocalHistograms.cs` computes histograms *per 15x15 block* (`Parameters.BlockSize=15`) via `BlockMap`/`BlockGrid`, not per pixel — O(blocks) not O(pixels·window²). Rewrote `LocalHistograms::new` as a two-pass O(w·h) separable sliding-window sum (horizontal pass then vertical pass, reusing running per-bin counts) — cut `local_histograms` from ~400-475ms to **~330-370ms** (modest gain; still per-pixel granularity, still far above .NET's per-block ~15ms scale). `cargo test --lib` 223/223 still passing after the rewrite. Full architectural fix requires switching `LocalHistograms`/`ImageEqualizer`/`BinarizedImage`/`SegmentationMask` to `BlockMap`/`BlockGrid`-based per-block histograms (types already exist in `src/primitives/block_map.rs` and `block_grid.rs`, unused by the extractor pipeline) — this is a real architectural change touching 4 files, held pending user approval per project convention (user requires confirmation before structural refactors). Dotnet-sdk-8.0 was installed on this host to obtain real .NET baseline timings/scores: probe extract ≈72-77ms warm. Rust warm timing after the histogram rewrite: ≈406ms/image (was ≈459ms before) — still ~5-6x slower than .NET, `local_histograms` remains the dominant cost. |
