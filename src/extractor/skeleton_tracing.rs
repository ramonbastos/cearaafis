...
impl SkeletonTracer {
    ...
    pub fn pixel_count(&self) -> usize {
        ...
        for y in 0..h {
            for x in 0..w {
                if self.skeleton.get(x, y) { count += 1; }
            }
        }
        count
    }
    ...
    pub fn find_minutia_points(&self) -> Vec<IntPoint> {
        ...
        for y in 0..h {
            for x in 0..w {
                if !self.skeleton.get(x, y) { continue; }
                ...
                for dy in -1i32..=1i32 {
                    for dx in -1i32..=1i32 {
                        ...
                        if nx >= 0 && ny >= 0 && nx < w as i32 && ny < h as i32 {
                            if self.skeleton.get(nx as usize, ny as usize) { n += 1; }
                        }
                    }
                }
...
mod tests {
    ...
    fn test_skeletonize_simple_line() {
        ...
        for x in 0..10 { binary.set(x, 5, true); }
    ...
    fn test_skeletonize_cross() {
        let mut binary = BooleanMatrix::new(10, 10);
        for x in 0..10 { binary.set(x, 5, true); }
        for y in 0..10 { binary.set(5, y, true); }
        let tracer = SkeletonTracer::new(&binary);
    ...
    fn test_pixel_count() {
        ...
        for y in 0..5 { for x in 0..5 { binary.set(x, y, true); } }
        let tracer = SkeletonTracer::new(&binary);
    ...
    fn test_find_minutia_points() {
        let mut binary = BooleanMatrix::new(10, 10);
        for y in 0..10 { for x in 0..10 { binary.set(x, y, true); } }
        let tracer = SkeletonTracer::new(&binary);
